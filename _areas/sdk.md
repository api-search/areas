...
- slug: fidel-api
  name: FIDEL API
  description: Fidel API is a global card-linking platform that connects Visa, Mastercard and Amex payment cards to web and mobile applications through a single API. Once a cardholder links a card (via Fidel's PCI-compliant Web, iOS, Android or React Native SDKs), Fidel streams enriched, real-time transaction dat…
  api_count: 1
  score_band: developing
...
overview: 'SDKs is one of the API Evangelist areas on the [APIs.io](https://apis.io/) network — a focused corner of the API landscape. The full area lives at [sdk.apievangelist.com](https://sdk.apievangelist.com).
  ...
  30 providers on the network work in this area, including Speakeasy, Dwolla, Android, Split, Chroma, Tripetto, and 24 more — each links out to that provider''s APIs, schemas, and governance artifacts.
